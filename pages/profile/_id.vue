<template>
    <div class="flex mb-4">
        ...
        <div v-if="!userLoading && user.id" class="w-full bg-white border border-grey p-5 rounded">
            <h1 class="mb-5">Hello {{ `${user.first_name} ${user.last_name}` }}</h1>
...
<script>
    ...
    export default {
        ...
        fetch({ store, params }) {
            store.dispatch('user/view', params.id)
